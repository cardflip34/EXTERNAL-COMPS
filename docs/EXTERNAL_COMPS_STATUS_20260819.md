...
- **Canonical store prototype** (DuckDB 1.5.5 + Parquet, `external_store/`): raw JSON → raw Parquet (3.86 GB → 1.18 GB, ~70 s) → `external_market_canonical_v1` **3,255,365 rows**, 378,441 dupes removed, conflicts price 1,465 / date 384 / image 912, price_missing 0, ~170 s canonical stage under a 1.4 GB cap, 1.1 GB partitioned Parquet by sold year/month, originals untouched. Report: `external_store/reports/migration_report_latest.json`.
